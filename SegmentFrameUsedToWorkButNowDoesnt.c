...
void on_trackbar(int){
	...
	//Create sequences to store the Normalized Boundaries (the ones that are respambled to be the same length
	...
	//Now we need to resample the left and right boundary to have the same number of points a min(L,R)
	...
	resampleSeq(centerline,&Ncenterline,NUMSEGMENTS);



	/*
	 * We need to loop through the points on the sampled centerline and
	 * find the closest points on the boundary so as to buld up an optimized Boundary
	 */
	...
	CvPoint NearestPtA;
	CvPoint NearestPtB;
	CvPoint* CenterLinePt;
	CvSeqReader reader;
	CvSeqReader readerLag;
	CvSeqWriter writerA;
	CvSeqWriter writerB;
	cvStartReadSeq(Ncenterline,&reader,0);
	cvStartReadSeq(Ncenterline,&readerLag,0);
	cvStartAppendToSeq(OptBoundA,&writerA);
	cvStartAppendToSeq(OptBoundB,&writerB);
	if (PRINTOUT) printf("Entering First point special case.\n");

	/*
	 * Special Case: The first point. Since eventually I am going to be mapping the area of the worm
	 * to a square, it makes sense to not have both contours and the centerline end on the same point.
	 * I treat the first and last point as special cases. In each instance when I am on the very
	 * first or last point of the centerline, I will arbitrarily chose one point over on teh boundary
	 */
	CvPoint* tempPt;
	tempPt=(CvPoint*) cvGetSeqElem(NBoundA,1);
	CV_WRITE_SEQ_ELEM( *tempPt, writerA);
	tempPt=(CvPoint*) cvGetSeqElem(NBoundB,1);
	CV_WRITE_SEQ_ELEM( *tempPt, writerB);

	/*
	 * Now we loop through
	 *
	 */
	//Set the Reader to be one point ahead of the ReaderLag
	CV_NEXT_SEQ_ELEM(sizeof(CvPoint),reader);

	CvPoint* CentLineVec;
	CvPoint* PrevCenterLinePt;
	if (PRINTOUT) printf("Entering Loop to find diagonal vector.\n");
	for (i=1;i< (Ncenterline->total -1);i++){
		CenterLinePt=(CvPoint*) reader.ptr;
		PrevCenterLinePt=(CvPoint*) readerLag.ptr;
		if (PRINTOUT) printf("CenterLinePt= (%d, %d)\t PrevCenterLinePt= (%d, %d)\n",CenterLinePt->x,CenterLinePt->y,PrevCenterLinePt->x,PrevCenterLinePt->y);
		*CentLineVec=cvPoint(CenterLinePt->x - PrevCenterLinePt->x,CenterLinePt->y - PrevCenterLinePt->y);
		if (PRINTOUT) printf("\tCenterLinePt=(%d,%d)\n",CenterLinePt->x,CenterLinePt->y);
		//Now Find the nearest neighbor on the boundary



		NearestPtA = FindNormalPt(CenterLinePt,CentLineVec,NBoundA);
		NearestPtB = FindNormalPt(CenterLinePt,CentLineVec,NBoundB);
		printf("FoundNormalPt\n");
		//Write the points to the Optimized Boundaries
		CV_WRITE_SEQ_ELEM( NearestPtA, writerA);
		CV_WRITE_SEQ_ELEM( NearestPtB, writerB);
		if (PRINTOUT) printf("NearestPtA=(%d,%d)\tNearestPtB=(%d,%d)\n",NearestPtA.x,NearestPtA.y,NearestPtB.x,NearestPtB.y);
		//Move to the Next point on the Centerline
		CV_NEXT_SEQ_ELEM(sizeof(CvPoint),reader);
		CV_NEXT_SEQ_ELEM(sizeof(CvPoint),readerLag);
		if (HEAVYDIAGNOSTICS) {
			cvCircle(g_gray, NearestPtA, 1, cvScalar(255, 255, 255), 1);
			cvCircle(g_gray, NearestPtB, 1, cvScalar(255, 255, 255), 1);
			cvCircle(g_gray, *CenterLinePt, 1, cvScalar(255, 255, 255), 1);
			cvLine(g_gray,NearestPtA,*CenterLinePt,cvScalar(255,255,255),1,CV_AA,0);
			cvLine(g_gray,NearestPtB,*CenterLinePt,cvScalar(255,255,255),1,CV_AA,0);
			cvWaitKey(100);
	...
	cvEndWriteSeq(&writerA);
	cvEndWriteSeq(&writerB);
	if (PRINTOUT) printf("Finished finding optimized boundaries\n");
	printf("OptBoundA->total=%d,OptBoundB->total=%d\n",OptBoundA->total,OptBoundB->total);
	if (PRINTOUT) cvWaitKey(0);

	//Display the points slowly on screen
	for (i = 0; i < Ncenterline->total; i++) {
		CvPoint* tempPt = (CvPoint*) cvGetSeqElem(Ncenterline, i);
		CvPoint* tempPtA = (CvPoint*) cvGetSeqElem(OptBoundA, i);
		CvPoint* tempPtB = (CvPoint*) cvGetSeqElem(OptBoundB, i);
		printf("tempPt=( %d,%d)\t tempPtA =( %d,%d)\t tempPtB=( %d,%d)\n",tempPt->x,tempPt->y,tempPtA->x,tempPtA->y,tempPtB->x,tempPtB->y);
...
////	 Draw lines connecting sequence
	...
	for (i=0;i<BoundarySideA->total;i++){
		...
		CvPoint* SideA=(CvPoint*)readerA.ptr;
		CvPoint* SideB=(CvPoint*)readerB.ptr;
